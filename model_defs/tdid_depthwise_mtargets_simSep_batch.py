...
class TDID(nn.Module):
    ...
    def __init__(self):
        ...
        #self.cc_conv1 = Conv2d(2*self.groups,512, 1, relu=False, same_padding=True)
        #self.diff_conv1 = Conv2d(2*512,512, 1, relu=False, same_padding=True)
        self.sim_conv1 = Conv2d(4*512,2*512, 3, relu=True, same_padding=True)
        self.sim_conv2 = Conv2d(2*512,512, 3, relu=True, same_padding=True)

        self.loc_conv1 = Conv2d(3*512,2*512, 3, relu=True, same_padding=True)
        self.loc_conv2 = Conv2d(2*512,512, 3, relu=True, same_padding=True)
    ...
    def forward(self, target_data, im_data, gt_boxes=None,features_given=False, im_info=None):
        ...
        sim = self.sim_conv1(torch.cat([diffs,cc],1))
        sim = self.sim_conv2(sim)

        loc = self.loc_conv1(torch.cat([img_features,cc],1))
        ...
        # rpn boxes
        rpn_bbox_pred = self.bbox_conv(loc)
