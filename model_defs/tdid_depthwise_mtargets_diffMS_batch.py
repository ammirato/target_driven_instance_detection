...
class TDID(nn.Module):
    ...
    def __init__(self):
        ...
        #self.conv1MS = Conv2d(2*self.groups+512,512, 3, relu=False, same_padding=True)
        #self.cc_convMS = Conv2d(2*self.groups,512, 3, relu=True, same_padding=True)
        #self.diff_convMS = Conv2d(2*self.groups,512, 3, relu=True, same_padding=True)
        #self.score_convMS = Conv2d(512, len(self.anchor_scales) * 3 * 2, 1, relu=False, same_padding=False)
        #self.bbox_convMS = Conv2d(512, len(self.anchor_scales) * 3 * 4, 1, relu=False, same_padding=False)
    ...
    def forward(self, target_data, im_data, gt_boxes=None, features_given=False, im_info=None):
        ...
        if not features_given:
            ...
            img_featuresMS = F.upsample(img_features,scale='2',mode='bilinear')      
        ...
        for b_ind in range(img_features.size()[0]):
            ...
            sample_imgMS = torch.index_select(img_featuresMS,0,img_ind)
            ...
            diff1MS = sample_img - tf1_pooled.permute(1,0,2,3).expand_as(sample_imgMS)
            diff2MS = sample_img - tf2_pooled.permute(1,0,2,3).expand_as(sample_imgMS)
            diffsMS.append(torch.cat([diff1MS,diff2MS],1))
            ...
            cc1MS = F.conv2d(sample_imgMS,sample_targets1,padding=padding,groups=self.groups) 
            cc2MS = F.conv2d(sample_imgMS,sample_targets2,padding=padding,groups=self.groups) 
            ccMS = torch.cat([cc1MS,cc2MS],1)
            ccMS = self.select_to_match_dimensions(ccMS,sample_imgMS)
            ccsMS.append(ccMS)
        ...
        ccMS = torch.cat(ccsMS,0)
        ccMS = self.cc_conv(ccMS)
        diffsMS = torch.cat(diffsMS,0) 
        diffsMS = self.diff_conv(diffsMS)
       
        cc = torch.cat([cc,img_features, diffs],1) 
        rpn_conv1 = self.conv1(cc)


        ccMS = torch.cat([ccMS,img_featuresMS, diffsMS],1) 
        rpn_conv1MS = self.conv1(ccMS)
        ...
        rpn_cls_scoreMS = self.score_conv(rpn_conv1MS)
        rpn_cls_score_reshapeMS = self.reshape_layer(rpn_cls_scoreMS, 2)
        rpn_cls_probMS = F.softmax(rpn_cls_score_reshapeMS)
        rpn_cls_prob_reshapeMS = self.reshape_layer(rpn_cls_probMS, len(self.anchor_scales)*3*2)
        ...
        rpn_bbox_predMS = self.bbox_conv(rpn_conv1MS)
        ...
        roisMS,scoresMS, anchor_indsMS, labelsMS = self.proposal_layer(rpn_cls_prob_reshapeMS, rpn_bbox_predMS,im_info,
                                            cfg_key, self._feat_stride/2, self.anchor_scales, gt_boxes)
