...
#        self.corr_bn = nn.BatchNorm2d(1) 
        ...
        self.diff_conv1 = Conv2d(512*self.num_targets,32, 3, relu=False, same_padding=True)
    ...
    def forward(self, targets_data, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None, target_features_given=False):
        ...
        for tf in target_features:
            ...
            tf_reshape = tf.view(tf.size()[0],tf.size()[1],-1)
            tf_pooled = F.max_pool1d(tf_reshape,tf_reshape.size()[2])
            diff = img_features - (tf_pooled.unsqueeze(3)).expand_as(img_features) 
            diffs.append(diff)
