...
class TDID(nn.Module):
    ...
    def __init__(self):
        ...
        #self.target_conv = Conv2d(512,256, 5,relu=True,same_padding=False)
        self.target_conv = Conv2d(512,256, 5,relu=True,same_padding=True)

        self.corr_conv1 = Conv2d(self.num_targets,32, 3, relu=False, same_padding=True)
        self.diff_conv1 = Conv2d(256*self.num_targets,32, 3, relu=False, same_padding=True)
        self.score_conv = Conv2d(64, len(self.anchor_scales) * 3 * 2, 1, relu=False, same_padding=False)
        self.bbox_conv = Conv2d(64, len(self.anchor_scales) * 3 * 4, 1, relu=False, same_padding=False)
    ...
    def forward(self, targets_data, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None, target_features_given=False):
        ...
        img_features_tc = self.target_conv(img_features) 
        ...
        for tf in target_features:
            ...
            #also subtract the features
            tf_tc = self.target_conv(tf)
            tf_tc = tf_tc[:,:,2,2].unsqueeze(2).unsqueeze(3)
            diff = img_features_tc - tf_tc.expand_as(img_features_tc)
            diffs.append(diff)

        #concatenate all the cross correlation features
        corr_cat_feats = torch.cat(cross_corrs, 1)
        diff_cat_feats = torch.cat(diffs, 1)



        corr_rpn_conv1 = self.corr_conv1(corr_cat_feats)
        diff_rpn_conv1 = self.diff_conv1(diff_cat_feats)
        rpn_conv1 = torch.cat([corr_rpn_conv1,diff_rpn_conv1],1)
