...
def anchor_target_layer(rpn_cls_score, gt_boxes, im_info, cfg, _feat_stride=[16, ],
    ...
    for batch_ind in range(batch_size):
        ...
        scores = rpn_cls_score[batch_ind,9:,:,:].reshape(-1)
        ...
        num_bg = min(cfg.PROPOSAL_BATCH_SIZE - np.sum(labels == 1), np.sum(labels == 1)*3) + 1
        bg_inds = np.where(labels == 0)[0]
        if len(bg_inds) > num_bg:

            #pick just highest scoring negatives
            labels[bg_inds] = -1
            bg_scores = scores[bg_inds]
            sorted_inds = np.argsort(bg_scores)
            chosen_inds = sorted_inds[len(sorted_inds)-num_bg:] 
            labels[chosen_inds] = 0
