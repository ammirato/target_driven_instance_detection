...
class TDID(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = Conv2d(2*self.groups,self.groups, 3, relu=True, same_padding=True)
        self.conv2 = Conv2d(self.groups,256, 3, relu=True, same_padding=True)
        self.conv13 = Conv2d(512,512, 3, relu=True, same_padding=True)
        self.conv23 = Conv2d(512,256, 3, relu=True, same_padding=True)
    ...
    def forward(self, target_data, im_data, gt_boxes=None):
        ...
        img_features3,img_features = self.features(im_data, return_intermediates=True)
       

 
        target_data = network.np_to_variable(target_data, is_cuda=True)
        target_data = target_data.permute(0, 3, 1, 2)
        target_features3,target_features = self.features(target_data, return_intermediates=True)
        ...
        for b_ind in range(img_features.size()[0]):
            ...
            sample_targets31 = torch.index_select(target_features3,0,target_inds[0])
            sample_targets32 = torch.index_select(target_features3,0,target_inds[1])
            sample_targets31 = F.max_pool2d(sample_targets32,2)
            sample_targets32 = F.max_pool2d(sample_targets32,2)
            img_ind = network.np_to_variable(np.asarray([b_ind]),
                                                is_cuda=True, dtype=torch.LongTensor)
            sample_img = torch.index_select(img_features,0,img_ind)
            sample_img3 = torch.index_select(img_features3,0,img_ind)
            ...
            sample_targets31 = sample_targets31.view(-1,1,sample_targets31.size()[2], 
                                                   sample_targets31.size()[3])
            sample_targets32 = sample_targets32.view(-1,1,sample_targets32.size()[2], 
                                                   sample_targets32.size()[3])
            ...
            cc31 = F.conv2d(sample_img3,sample_targets31,padding=padding,groups=self.groups/2) 
            cc32 = F.conv2d(sample_img3,sample_targets32,padding=padding,groups=self.groups/2) 

            cc = torch.cat([cc1,cc2],1)
            cc3 = torch.cat([cc31,cc32],1)
            cc3 = F.max_pool2d(cc3,2)
            cc = self.select_to_match_dimensions(cc,sample_img)
            cc3 = self.select_to_match_dimensions(cc3,sample_img)
            #ccs.append(torch.cat([cc,cc3],1))
            ccs.append(cc)
            cc3s.append(cc3)

        cc = torch.cat(ccs,0)
        cc3 = torch.cat(cc3s,0)  
 
        rpn_conv1 = self.conv1(cc)
        rpn_conv1 = self.conv2(rpn_conv1)

        rpn_conv13 = self.conv13(cc3)
        rpn_conv13 = self.conv23(rpn_conv13)

        rpn_conv1 = torch.cat([rpn_conv1,rpn_conv13],1)
