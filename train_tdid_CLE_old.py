...
# load config
cfg_file = 'configCLE' #NO FILE EXTENSTION!
...
def validate_and_save(cfg,net,valset,valset_test_only,target_images, epoch, total_iterations):
    ...
    valloader_test_only = torch.utils.data.DataLoader(valset_test_only,
                                          batch_size=1,
                                          shuffle=True,
                                          collate_fn=AVD.collate)
    ...
    all_results_test_only = test_net(model_name, net, valloader_test_only, 
    ...
    if len(all_results_test_only) == 0:
        #coco code can't handle no detections?
        m_ap_test_only = 0
    else:
        m_ap_test_only = coco_det_eval(cfg.VAL_GROUND_TRUTH_BOXES,
                             cfg.TEST_OUTPUT_DIR+model_name+'.json',
                             catIds=cfg.TEST_ONLY_OBJ_IDS)

    save_name = os.path.join(cfg.SNAPSHOT_SAVE_DIR, 
                             (cfg.MODEL_BASE_SAVE_NAME+
                              '_{}_{}_{:1.5f}_{:1.5f}_{:1.5f}.h5').format(epoch,
                             total_iterations, epoch_loss/epoch_step_cnt,m_ap,m_ap_test_only))
...
test_only_ids = check_object_ids(cfg.TEST_ONLY_OBJ_IDS, cfg.ID_TO_NAME,target_images) 
assert(cfg.TEST_ONLY_OBJ_IDS == test_only_ids)
if train_ids==-1 or val_ids==-1 or test_only_ids==-1:
    print('Invalid IDS!')
    sys.exit()
...
for epoch in range(1,cfg.MAX_NUM_EPOCHS+1):
    ...
    for step,batch in enumerate(trainloader):
        ...
        if total_iterations == cfg.MAX_NUM_ITERATIONS:
            optimizer = torch.optim.SGD(params, lr=cfg.LEARNING_RATE*.1,
                                                momentum=cfg.MOMENTUM, 
                                                weight_decay=cfg.WEIGHT_DECAY)
        if total_iterations == 2*cfg.MAX_NUM_ITERATIONS:
            sys.exit()
...
####################################################
            batch_scene_imgs,batch_gt_boxes,batch_target_imgs = avd_extra_loader.get_batch(cfg.BATCH_SIZE)
...
####################################################
        ...
        elif cfg.USE_VID > np.random.rand():
            #print('VID!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
...
####################################################
    ...
    #    validate_and_save(cfg,net,valset,valset2,target_images, epoch, total_iterations)
