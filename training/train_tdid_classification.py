...
from instance_detection.model_defs.TDID_class6 import TDID
...
from instance_detection.testing.test_tdid_classification import test_net, im_detect
from instance_detection.evaluation.COCO_eval.coco_det_eval import coco_det_eval 

import active_vision_dataset_processing.data_loading.active_vision_dataset_pytorch as AVD  

# load config
#cfg_file = 'configUW_class' #NO FILE EXTENSTION!
cfg_file = 'config1shotUW' #NO FILE EXTENSTION!
...
def validate_and_save(cfg,net,valset,target_images, epoch, total_iterations):
    ...
    accuracy  = test_net(model_name, net, valloader, cfg.ID_TO_NAME, 
    ...
    save_name = os.path.join(cfg.SNAPSHOT_SAVE_DIR, 
                             ...
                             epoch, total_iterations, epoch_loss/epoch_step_cnt, accuracy))
...
for epoch in range(cfg.MAX_NUM_EPOCHS):
    ...
    for step,batch in enumerate(trainloader):
        ...
        for sample_ind in range(cfg.BATCH_SIZE):
            ...
            if cfg.AUGMENT_SCENE_IMAGES and np.random.rand() < .8:
                im_data = vary_image(im_data,crop_max=30, do_illum=False)
            im_data=normalize_image(im_data,cfg)
            object_present = batch[1][sample_ind][0][0]
            not_present = np.asarray([ind for ind in train_ids 
                                              if ind !=object_present and 
                                                 ind != 0]) 

            #pick a random target, with a bias towards choosing a target that 
            #is in the image. Also pick just that object's gt_box
            if (np.random.rand() < .7 or not_present.shape[0]==0):
                target_ind = object_present
            ...
            for t_type,_ in enumerate(target_images[target_name]):
                ...
                if np.random.rand() < .8 and cfg.AUGMENT_TARGET_IMAGES:
            ...
            batch_cids.append(object_present)
        ...
        gt_cids = network.np_to_variable(gt_cids,is_cuda=True, dtype=torch.cuda.LongTensor) 

        # forward
        net(target_data, im_data, gt_cids=gt_cids, im_info=im_info)
