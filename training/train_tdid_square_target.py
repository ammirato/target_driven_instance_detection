...
from instance_detection.model_defs.tdid_many_measures_square_target import TDID 
...
output_dir = ('/net/bvisionserver3/playpen/ammirato/Data/Detections/' + 
             '/saved_models/')
save_name_base = 'TDID_archMMST_0'
save_freq = 2 
...
excluded_cids = [16]#[53, 1,2,18,21,25]
chosen_ids =  [x for x in range(0,28) if x not in excluded_cids]
val_chosen_ids = range(0,28)#for validation testing
...
target_path = '/net/bvisionserver3/playpen/ammirato/Data/instance_detection_targets/AVD_BB_exact_few_square/'
...
val_chosen_ids = list(set(set(val_chosen_ids) & set(name_to_id.values())))
...
for epoch in range(num_epochs):
    ...
    if epoch % save_freq == 0:
        ...
        save_name = os.path.join(output_dir, save_name_base+'_{}_{:1.5f}_{:1.5f}.h5'.format(save_epoch, epoch_loss/epoch_step_cnt, m_ap))
