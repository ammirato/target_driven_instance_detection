...
from instance_detection.model_defs.tdid_batch import TDID 
...
save_name_base = 'TDID_COMB_archD_ROI_0'
save_freq = 2000
...
trained_model_path = ('/net/bvisionserver3/playpen/ammirato/Data/Detections/' +
                     '/saved_models/')
trained_model_name = 'TDID_COMB_archDDpbn_ROI_2_2_642.61764_0.00647_0.00049.h5'
...
val_chosen_ids = [[4,5,17,19],[5,11,12,14]] #, [5,10,17]]#chosen_ids #range(0,28)#for validation testing
...
target_size = [200,16]
#CREATE TRAIN/TEST splits
train_set_VID = VID_Loader(VID_data_path,'train_single', target_size=target_size)
...
for step in range(1,max_iterations):
    ...
    #only do AVD every other 'step'

    if step % 2 == 0:

        ind = np.random.choice(len(train_set),1)[0]

        batch = train_set[ind:ind+2]
        ...
        if max(im_data.shape) > 1000:
            resize = .5
            im_data = cv2.resize(im_data,(int(im_data.shape[1]*resize), int(im_data.shape[0]*resize)))
        ...
        if max(im_data2.shape) > 1000:
            resize2 = .5
            im_data = cv2.resize(im_data,(int(im_data.shape[1]*resize2), int(im_data.shape[0]*resize2)))
        ...
        if (np.random.rand() < .8 or not_present.shape[0]==0) and objects_present.shape[0]!=0:
            ...
            gt_boxes = gt_boxes[np.where(gt_boxes[:,4]==target_ind)[0],:-1] * resize
            gt_boxes[0,4] = 1
        ...
        if target_ind in objects_present2:
            gt_box2 = gt_boxes2[np.where(gt_boxes2[:,4]==target_ind)[0],:-1] * resize2
            gt_boxes[0,4] = 1
        else:
    ...
    if step % disp_interval == 0:
        ...
        log_text = 'step %d, epoch_avg_loss: %.4f, fps: %.2f (%.2fs per batch) tv_cnt:%d' \
            ...
            step,  epoch_loss/epoch_step_cnt, fps, 1./fps, tv_cnt, ir_cnt, step, loss.data[0],train_loss/step_cnt)
    ...
    if step % save_freq == 0:
        save_epoch = step
        if load_trained_model:
            save_epoch = step+trained_epoch+1
